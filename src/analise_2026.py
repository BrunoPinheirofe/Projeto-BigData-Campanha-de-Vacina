...
def analisar_2026(apenas_ac=True):
    print("Iniciando carregamento do PNI para o ano de 2026...")
    # Para demonstração e rapidez, vamos carregar apenas o Acre (AC) conforme validado pelo projeto
    # Se quiser o Brasil inteiro, mude 'apenas_ac' para False (demora consideravelmente mais).
    ufs_alvo = ["AC"] if apenas_ac else None
    
    try:
        t_pni = load_pni_ufs(ufs_alvo if ufs_alvo else ["AC"], anos=["2026"])
        ...
        if PYSUS_DISPONIVEL:
            print("\nBaixando dados do SIH e SIM (casos de gripe) para o ano de 2026 via PySUS...")
            ...
            if sih_dfs:
                ...
                # O SIH não tem a data exata facilmente mapeável sem tratar, mas podemos extrair o mês da internação
                # Assumindo que SP_DTINTER tem a data. Para o propósito do cruzamento:
                if "SP_DTINTER" in df_sih_influenza.columns:
                    ...
                    df_sih_influenza["ano_vacina"] = 2026
                    df_sih_influenza["mes"] = 1 # simplificação se não houver data
            ...
            if sim_dfs:
                ...
                if "DTOBITO" in df_sim_influenza.columns:
                    ...
                    df_sim_influenza["ano_vacina"] = 2026
        ...
        # Opcional: calcular gap score para identificar áreas críticas
        ...
        print("\nExportando resultado para 'resultado_cruzamento_2026.csv'...")
        df_cruzado.to_csv("resultado_cruzamento_2026.csv", index=False)
        print("Finalizado!")
